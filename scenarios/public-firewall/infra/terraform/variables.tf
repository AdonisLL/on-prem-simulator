...
variable "deployer_address_prefix" {
  ...
  validation {
    condition = (
      can(cidrhost(var.deployer_address_prefix, 0)) &&
      length(split(".", split("/", var.deployer_address_prefix)[0])) == 4 &&
...
variable "enable_temporary_deployment_access" {
  type        = bool
  description = "Temporarily permits deployer and firewall-mediated public access to Key Vault and Storage. Default remains private/default-deny."
  default     = false
}
...
variable "address_prefixes" {
  ...
  default = {
    ...
    web               = "10.50.3.0/24"
    data              = "10.50.4.0/24"
    private_endpoints = "10.50.5.0/27"
  }
}
